...
namespace Weiguang.Core
{
    public class RevealThresholdTracker
    {
        ...
        public int Update(float pct, Action<float> onThreshold)
        {
            ...
            if (clamped >= T25 && !_fired25) { _fired25 = true; onThreshold?.Invoke(T25); fired++; }
            if (clamped >= T50 && !_fired50) { _fired50 = true; onThreshold?.Invoke(T50); fired++; }
            if (clamped >= T75 && !_fired75) { _fired75 = true; onThreshold?.Invoke(T75); fired++; }
