...
5. **读数**（任一方式）：
   ...
   - **输出行的字段结构**（键固定，值由真机实跑产出——下面只列字段骨架，**非实测数字**）：
     ```
     [Analytics] device_fps {
       "avg_fps":<float>, "min_fps":<float>,
       "device_model":"<SystemInfo.deviceModel>",
       "quality_tier":"<low|mid|high>",
       "sample_sec":60, "frames":<int>,
...
> 🟡 **状态：模板/链路就绪，A1 锁 🟡 待真机补录（当前无真·低端机，C1 验收物理不可达）**。沙箱无 GPU/设备，主程无法产出 FPS 数字；用户当前亦无 ≤1024MB 真机，故 C1（avg≥30）验收项暂不可执行。请按 §3 在真·低端机（≤1024MB）跑满窗口后，把下表数字抄入（也可直接贴 `device_fps` 事件行 / `adb logcat` 抓取行） —— 届时再翻 ✅。

> ⛔ **回填必须来自真机 logcat 实跑行**；§3 的"输出行的字段结构"只是字段骨架（`<...>` 占位），含任何具体数字的示例都**不是实测**，不得据此填写本表。
...
待真·低端机（≤1024MB）实测回填后，把本表状态从 🟡 翻 ✅，并同步更新 `release/phase8-release-checklist.md` 的 A1 项。当前因无该设备，A1 维持 🟡、不翻 ✅（绝不以 Pixel 4a 等 ≥1536MB 机型冒充 C1 验收）。
